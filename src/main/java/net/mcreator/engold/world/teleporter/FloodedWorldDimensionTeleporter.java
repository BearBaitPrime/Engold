...
public class FloodedWorldDimensionTeleporter {
	public static Holder<PoiType> poi = null;

	public static void registerPointOfInterest() {
		PoiType poiType = PoiHelper.register(Identifier.parse("engold:flooded_world_dimension_portal"), 0, 1, ImmutableSet.copyOf(EngoldModBlocks.FLOODED_WORLD_DIMENSION_PORTAL.getStateDefinition().getPossibleStates()));
		poi = BuiltInRegistries.POINT_OF_INTEREST_TYPE.wrapAsHolder(poiType);
	}

	private final ServerLevel level;

	public FloodedWorldDimensionTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos origin, Direction.Axis portalAxis) {
			...
			for (int box = -1; box < 2; ++box) {
				for (int width = 0; width < 2; ++width) {
					for (int height = -1; height < 3; ++height) {
						BlockState blockState = height < 0 ? Blocks.CRYING_OBSIDIAN.defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		BlockState portalBlockState = (BlockState) EngoldModBlocks.FLOODED_WORLD_DIMENSION_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, portalAxis);
		for (int width = 0; width < 2; ++width) {
